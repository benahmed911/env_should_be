...
@file_to_dictionary
def load_env_file(file: TextIOWrapper = None) -> dict:
    if not file:
        raise SystemError()
    env_vars = {}
    for line in file:
...
@file_to_dictionary
def load_yaml_file(file: TextIOWrapper = None) -> dict:
    if not file:
        raise SystemError()
...
@file_to_dictionary
def load_json_file(file: TextIOWrapper = None) -> dict:
    if not file:
        raise SystemError()
    return load(file)
...
def is_valid_env(expected_env: dict, actual_env: dict) -> bool | None:
    ...
    for key, values in expected_env.items():
        ...
        for name, klass in inspect.getmembers(all_descriptions):
            ...
            if not (value and description.does_pass(value)):
        ...
        try:
            if extension == ".json":
                ...
                raise FileHasNoExtension(
                    "make sure the description file ends with: .json/.yaml/.yml"
                )
        except (
            FileNotFoundError,
            JSONDecodeError,
